...
custom_code_dir = os.path.dirname(os.path.abspath(__file__))
download_dir = custom_code_dir
...
slip_weight_paths = {key: os.path.join('{}/slip_weights'.format(download_dir), value.split('/')[-1])
...
def get_slip_model(model_name, verbose = True):
    ...
    if not os.path.exists(slip_weight_paths[model_name]):
        download_url(slip_model_weights[model_name], '{}/slip_weights'.format(download_dir))
...
def get_seer_model(model_name):
    ...
    weights_dir = '{}/vissl_weights/'.format(download_dir)
    os.makedirs(weights_dir, exist_ok = True)
    
    weights_path = '{}/{}.torch'.format(weights_dir, model_name)
    if not os.path.exists(weights_path):
        download_url(seer_model_weights[model_name], download_dir, weights_path)
...
vqgan_paths = {'vqgan_imagenet_f16_16384': 
               {'config': f'{custom_code_dir}/vqgans_weights/vqgan_imagenet_f16_16384/config.yaml',
                'checkpoint': f'{custom_code_dir}/vqgans_weights/vqgan_imagenet_f16_16384/model.ckpt'}}
...
def get_vqgan_model(model_name):
    sys.path.append('{}/vqgans_codebase'.format(custom_code_dir))
